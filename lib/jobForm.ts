...
export const validateJobFormValues = (values: JobFormValues) => {
    ...
    if(!lengthIsBetween(values.jobTitle,3,50)) {
        errors.jobTitle = 'Job title must constain between 3 and 50 characters';
    }
    ...
    if(!lengthIsBetween(values.description,150,1500)) {
        errors.description = 'Description must contain between 150 and 1500 characters';
    }
...
const lengthIsBetween = (value: string, min: number, max: number) => {
